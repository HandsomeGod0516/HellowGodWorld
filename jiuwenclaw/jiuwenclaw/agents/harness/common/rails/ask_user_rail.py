...
EXTENDED_INPUT_PARAMS_CN: dict[str, Any] = {
    ...
    "properties": {
        "query": {
            "type": "string",
            "description": "向使用者展示的問題（必填）。",
        },
        "questions": {
            "type": "array",
            "description": (
                "帶選項的結構化問題。當希望使用者從預定義選項中選擇而非自由輸入時使用。"
                "每個問題必須提供 2-4 個選項。使用者始終可以選擇「其他」進行自定義輸入。"
            ),
            "items": _QUESTIONS_ITEM_SCHEMA,
        },
    },
...
_EXTENDED_DESCRIPTION_CN: str = (
    "中斷執行並向使用者請求輸入。支援兩種模式：\n"
    "1. 純文字查詢：只傳 `query` —— 使用者自由輸入回答。\n"
    "2. 結構化選項：傳 `query` + `questions` —— 使用者從預定義選項中選擇。"
    "當你希望使用者在特定選項間做選擇時（如「應用更新」vs「跳過」）使用 `questions`。"
    "每個問題可提供 2-4 個選項。"
)
