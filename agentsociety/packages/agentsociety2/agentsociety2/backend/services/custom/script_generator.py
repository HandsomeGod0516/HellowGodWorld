...
class SafeModuleTester:
    ...
    def _safe_import_class(self, module_path: str, class_name: str) -> Optional[Type]:
        if not self._validate_module_path(module_path):
            raise ValueError(
                f"模組路徑不在白名單內: {module_path}. "
                f"允許的字首: {self.ALLOWED_PATH_PREFIXES}"
            )
        if not class_name.replace("_", "").isalnum():
            raise ValueError(f"類名包含非法字元: {class_name}")
        ...
        try:
            if module_import_path.startswith("custom."):
                ...
                if spec is None or spec.loader is None:
                    raise ImportError(f"無法為 {file_path} 建立匯入 spec")
            ...
            if not inspect.isclass(cls):
                raise ValueError(f"{class_name} 不是一個類")
            return cls
        except ImportError as exc:
            raise ImportError(f"無法匯入模組 {module_import_path}: {exc}") from exc
        except AttributeError as exc:
            raise AttributeError(f"模組中沒有類 {class_name}: {exc}") from exc
    ...
    def _test_agent_class(self, cls: Type, class_name: str) -> TestResult:
        ...
        try:
            agent = cls(id=0, profile={"name": "測試", "personality": "友好"})
            output_lines.append("✓ 建立成功")
            checks.append(self._check("instantiation", True, "Agent 例項化成功"))
        except Exception as exc:
            output_lines.append(f"✗ 建立例項失敗: {exc}")
            checks.append(
                self._check("instantiation", False, f"Agent 例項化失敗: {exc}")
            )
            return self._finalize_result(
                name=class_name,
                module_kind="agent",
        ...
        if hasattr(agent, "mcp_description"):
            try:
                ...
                output_lines.append(f"✓ mcp_description() 返回 {len(description)} 字元")
                checks.append(
                    self._check(
                        "mcp_description",
                        bool(description),
                ...
                output_lines.append(f"✗ mcp_description() 呼叫失敗: {exc}")
                checks.append(
                    self._check("mcp_description", False, f"mcp_description() 呼叫失敗: {exc}")
                )
    ...
    def _test_env_class(self, cls: Type, class_name: str) -> TestResult:
        ...
        try:
            env = cls()
            output_lines.append("✓ 建立成功")
            checks.append(self._check("instantiation", True, "cls() 例項化成功"))
        except Exception as exc:
            output_lines.append(f"✗ 建立例項失敗: {exc}")
            checks.append(self._check("instantiation", False, f"cls() 例項化失敗: {exc}"))
            return self._finalize_result(
                name=class_name,
        ...
        output_lines.append(f"✓ 已註冊 {len(tool_names)} 個工具")
        checks.append(
            self._check(
                "registered_tools",
                len(tool_names) > 0,
                f"已註冊 {len(tool_names)} 個工具" if tool_names else "未註冊任何工具",
        ...
        checks.append(
            self._check(
                "step",
                has_step,
                "step() 方法已覆寫" if has_step else "step() 方法未覆寫 EnvBase.step",
            )
        )
        output_lines.append(
            f"{'✓' if has_step else '✗'} step() 方法{'已覆寫' if has_step else '未覆寫 EnvBase.step'}"
        )
        ...
        try:
            ...
            checks.append(
                self._check(
                    "router_smoke",
                    True,
                    "CodeGenRouter 可正常掛載該環境",
                )
            )
        except Exception as exc:
            output_lines.append(f"✗ router smoke test 失敗: {exc}")
            checks.append(
                self._check("router_smoke", False, f"router smoke test 失敗: {exc}")
            )
    ...
    def _test_integration(
        ...
        try:
            ...
            output_lines.append("⚠ 無法匯入 CodeGenRouter，跳過整合測試")
            checks.append(
                self._check("router_import", True, "CodeGenRouter 不可用，跳過整合測試")
            )
            return self._finalize_result(
        ...
        try:
            env = env_cls()
            router = CodeGenRouter(env_modules=[env])
            output_lines.append("✓ 環境路由建立成功")
            checks.append(self._check("router_creation", True, "環境路由建立成功"))

            agent = agent_cls(
                id=0,
                profile={"name": "整合測試", "personality": "測試"},
            )
            output_lines.append("✓ Agent 建立成功")
            checks.append(self._check("agent_creation", True, "Agent 建立成功"))
            ...
            if hasattr(agent, "init"):
                try:
                    ...
                    output_lines.append("✓ Agent 環境初始化成功")
                    checks.append(self._check("agent_init", True, "Agent 環境初始化成功"))
                except Exception as exc:
                    output_lines.append(f"✗ Agent 環境初始化失敗: {exc}")
                    checks.append(self._check("agent_init", False, f"Agent 環境初始化失敗: {exc}"))
        except Exception as exc:
            output_lines.append(f"✗ 整合測試失敗: {exc}")
            checks.append(self._check("integration", False, f"整合測試失敗: {exc}"))
            return self._finalize_result(
                name=f"{agent_name}+{env_name}",
                module_kind="integration",
                output_lines=output_lines,
    ...
    async def run_target_test(
        ...
        *,
        ...
        if not class_name:
            return {
                "success": False,
                "error": "必須提供 class_name 才能執行目標測試",
                "stdout": "",
                "stderr": "",
                "returncode": -1,
                "results": [],
        ...
        try:
            ...
            if cls is None:
                raise ValueError(f"無法匯入類 {class_name}")
            result = (
    ...
    def _serialize_results(
        ...
        *,
        ...
        return {
            ...
            "passed_tests": passed,
            "failed_tests": failed,
            "error": error if error is not None else (None if failed == 0 else f"{failed} 個測試失敗"),
        }
    ...
    async def run_test(self, scan_result: dict[str, Any]) -> dict[str, Any]:
        ...
        try:
            with redirect_stdout(stdout_buffer), redirect_stderr(stderr_buffer):
                all_output.extend(["=" * 50, "開始測試自定義模組", "=" * 50])

                if agents:
                    all_output.extend(["", "測試 Agent", "-" * 30])
                ...
                if agents and envs:
                    ...
                    try:
                        ...
                        if agent_cls is None or env_cls is None:
                            raise ValueError("無法匯入 Agent 或環境類")
                        ...
                        all_output.append(f"✗ 整合測試: {exc}")

                all_output.extend(["", "=" * 50, "測試完成", "=" * 50])
